    ...
    .createServer(function (request, response) {
        ...
        try {
            ...
            if (request.method === 'GET') {
                ...
                const etag = crypto.createHash('md5').update(data).digest('hex') // 获取返回参数的 hash 值
                ...
                if (request.headers['if-none-match'] === etag) { // 若本次返回值的 hash 信息，与请求头的 if-none-match hash 值一致，则直接返回 304，复用浏览器缓存的数据，无需要额外返回数据，节省带宽
                    ...
                    response.end(data); // 返回业务数据
                }
            } else if (request.method === 'POST') {
                let postData = '';
                request.on('data', chunk => postData += chunk.toString());
                request.on('end', () => {
                    const etag = crypto.createHash('md5').update(postData).digest('hex')
